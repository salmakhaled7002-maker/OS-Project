...
namespace OS_Project.Forms
{
    public partial class SJFPreemptiveForm : Form
    {
        ...
        private void btnDelete_Click(object sender,EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select a process to delete.","Delete Process",
        ...
        private void btnCalculate_Click(object sender,EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please add at least one process first.","No Processes",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
            ...
            try
            {
                ...
                List<Process> result = SJFPreemptive.Schedule(processes);
                for (int i = 0; i < result.Count;i++)
                {
                    dgvProcesses.Rows[i].Cells[3].Value =result[i].CompletionTime;
                    dgvProcesses.Rows[i].Cells[4].Value =result[i].TurnaroundTime;
                    dgvProcesses.Rows[i].Cells[5].Value =result[i].WaitingTime;
                }
                double averageWaitingTime = StatisticsCalculator.CalculateAverageWaitingTime(result);
                double averageTurnaroundTime = StatisticsCalculator.CalculateAverageTurnaroundTime(result);
                lblAverageWaiting.Text = "Average Waiting Time: " + averageWaitingTime
                    .ToString("0.##");
                lblAverageTurnaround.Text ="Average Turnaround Time: " + averageTurnaroundTime
                    .ToString("0.##");
                DrawGanttChart();
            }
            catch (Exception ex)
        ...
        {
            pnlGanttChart.Controls.Clear();
            if (SJFPreemptive.GanttChart == null || SJFPreemptive.GanttChart.Count == 0)
            {
                pnlGanttChart.AutoScrollMinSize = new Size(0, 0);
                pnlGanttChart.Invalidate();
                return;
            ...
            {
                if (i == SJFPreemptive.GanttChart.Count || SJFPreemptive.GanttChart[i] != currentProcess)
                {
        ...
        private void SJFPreemptiveForm_Load(object sender,EventArgs e)
        {
        }
